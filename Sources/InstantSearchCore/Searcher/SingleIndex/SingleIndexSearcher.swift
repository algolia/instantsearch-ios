...
//      }
//    } else {
//      let indexedQuery = IndexedQuery(indexName: indexQueryState.indexName, query: query)
//      operation = client.multipleQueries(queries: [indexedQuery]) { [weak self] result in
//        guard let searcher = self, searcher.query == query.query else { return }
//        searcher.processingQueue.addOperation {
//          let indexName = searcher.indexQueryState.indexName
//
