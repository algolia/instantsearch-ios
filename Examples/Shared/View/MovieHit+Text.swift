...
extension Hit where T == Movie {
  ...
  var genresText: Text {
    if let highlightedGenres = highlightResult?.value(forKey: "genres"), case .array(let hg) = highlightedGenres {
      let highlightedTexts = hg.compactMap({ $0.value }).map(\.value).map { Text(highlightedString: $0, highlighted: { Text($0).foregroundColor(Color(.algoliaCyan)) }) }
